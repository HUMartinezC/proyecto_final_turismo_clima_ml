...
| Amazon RDS MariaDB | Preparado/provisionable | Queda disponible como base relacional para resultados estructurados o pruebas; no se usa como almacen principal porque el dataset analitico vive en S3/Athena. |
| AWS Lambda | Preparada como automatizacion opcional | El flujo principal es batch, reproducible y se ejecuta desde el script unico. |
...
El procesamiento en tiempo real con Kafka no encaja de forma natural con las fuentes integradas. Dataestur, Open-Meteo, festivos y AENA son fuentes historicas o batch: se descargan por rango temporal, se normalizan y se agregan a nivel mensual.
...
DocumentDB se mantiene tambien como componente no implementado en el flujo actual. Las respuestas originales quedan conservadas en `datasets/raw/` y S3 bronze.

RDS y Lambda se mantienen fuera también. RDS podria almacenar logs de inferencia o resultados de ejecuciones, y Lambda podria programar disparadores de ingesta, pero se prioriza una arquitectura reproducible mediante S3, Glue, Athena y el script unico.
...
El objetivo es construir una union justificada. Dataestur aporta el target, Open-Meteo explica condiciones climaticas, `holidays` introduce estacionalidad y AENA añade movilidad aeroportuaria.
